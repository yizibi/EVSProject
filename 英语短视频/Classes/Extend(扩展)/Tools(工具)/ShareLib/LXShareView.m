...
#import "LXShareItemCell.h"

@interface LXShareView()<UICollectionViewDelegate,UICollectionViewDataSource>
...
-(void)layoutSubviews {
    ...
    //动画前置控件位置
    if (_containView) {
        _containView.frame = CGRectMake(0, SCREEN_HEIGHT-64, SCREEN_WIDTH, height);
    }
    if (_headerView) {
...
- (void)showInView:(UIView *)view{
    ...
    [shareCollectionView registerClass:[LXShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_LXShareItemCell];
    ...
    if (self.functionItems) {
        ...
        [functionCollectionView registerClass:[LXShareItemCell class] forCellWithReuseIdentifier:kCellIdentifier_LXShareItemCell];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LXShareItemCell *shareItemCell = [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier_LXShareItemCell forIndexPath:indexPath];
    ...
    if (collectionView == self.shareCollectionView) {
        if ([self.shareItems[indexPath.row] isKindOfClass:[NSString class]]) {
            shareItemCell.shareItem = [[LXShareItem alloc] initWithPlatformName:self.shareItems[indexPath.row]];
        }else{
        ...
        if ([self.functionItems[indexPath.row] isKindOfClass:[NSString class]]) {
            shareItemCell.shareItem = [[LXShareItem alloc] initWithPlatformName:self.functionItems[indexPath.row]];
        }else{
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    LXShareItemCell *cell = (LXShareItemCell *)[collectionView cellForItemAtIndexPath:indexPath];
